...
// POST /admin/generate-withdraw-code/:id
router.post('/generate-withdraw-code/:id', ensureAdmin, async (req, res) => {
    try {
        ...
        // telegramBot → notifies ADMIN that a code was generated
        ...
        try {
            await emailService.sendEmail({
                to: user[0].email,
                subject: `Your ${siteName} Withdrawal Code`,
                html: emailTemplate({
                    badge: 'WITHDRAWAL CODE',
                    badgeColor: '#22c55e',
                    username: user[0].username,
                    intro: 'your withdrawal code has been generated. Use it to process your withdrawal request.',
                    amountDisplay: code,
                    amountColor: '#22c55e',
                    rows: [
                        ['Code Expires', expiresAt.toUTCString(), '#f4f4f5'],
                        ['Generated By', req.session.user.username, '#f4f4f5'],
                        ['Date & Time', new Date().toUTCString(), '#f4f4f5']
                    ],
                    footerNote: 'This code expires in 24 hours. Do not share it with anyone. Contact support if you did not request this.',
                    siteName: siteName
                })
            });     
        } catch (emailErr) { 
...
router.post('/warnings/send', ensureAdmin, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            await emailService.sendEmail({
                to: user.email,
                subject: `Warning from ${siteName} — ${title}`,
                html: messageTemplate({
                    badge: 'Warning',
                    badgeColor: '#f59e0b',
                    username: user.username,
                    intro: `<strong>${title}</strong><br><br>${message}`,
                    rows: [
                        ['From', 'Support Team', '#111827'],
                        ['Date', new Date().toLocaleString(), '#111827']
                    ],
                    ...
                    siteName: siteName,
                    siteUrl: req.protocol + '://' + req.get('host'),
                    buttonText: action_text || 'View Details',
                    buttonUrl: action_url || '/messages'
                })
            });
        } catch (emailErr) {
